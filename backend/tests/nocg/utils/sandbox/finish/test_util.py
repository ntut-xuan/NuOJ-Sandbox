...
class TestTaskFinish:
    def test_with_test_task_should_change_the_task_status(self, cleanup_test_sandbox: None, test_task: Task):
        finish_task(test_task)
        
        assert test_task.status == StatusType.FINISH

    @freeze_time("2002-06-25 00:00:00")
    def test_with_test_task_should_have_running_timestamp(self, cleanup_test_sandbox: None, test_task: Task):
        finish_task(test_task)
