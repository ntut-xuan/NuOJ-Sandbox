...
def tc_upload():
	...
	# write file to back
	path = "/etc/nuoj-sandbox/storage/%s/%s" % (TunnelCode.TESTCASE.value, problem_pid + ".json")
...
def tc_fetch(problem_pid):
	...
	用於取得測資的部分，以及取得測資的數量
	'''
	if storage_util.file_storage_tunnel_exist(problem_pid + ".json", TunnelCode.TESTCASE) == False:
		return Response(json.dumps({"status": "Failed", "message": "testcase " + problem_pid + " not exist"}), mimetype="application/json")

	testcase_raw = storage_util.file_storage_tunnel_read(problem_pid + ".json", TunnelCode.TESTCASE)
	testcase_json_data = None
	
	try:
		testcase_json_data = json.loads(testcase_raw)
	except ValueError:
		return Response(json.dumps({"status": "Failed", "message": "testcase " + problem_pid + " is broken"}), mimetype="application/json")
